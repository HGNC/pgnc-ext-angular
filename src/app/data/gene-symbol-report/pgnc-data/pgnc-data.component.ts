...
export class PgncDataComponent implements OnInit {
    ...
    parseReport() {
        this.appSymbol = this.result.data?.geneSymbols?.find((geneSymbol) => { return geneSymbol.type === 'approved'; });
        this.appName = this.result.data?.geneNames?.find((geneName) => { return geneName.type === 'approved'; });
        this.prevSymbols = this.result.data?.geneSymbols?.filter((geneSymbol) => { return geneSymbol.type === 'previous'; });
        this.prevNames = this.result.data?.geneNames?.filter((geneName) => { return geneName.type === 'previous'; });
        this.aliasSymbols = this.result.data?.geneSymbols?.filter((geneSymbol) => { return geneSymbol.type === 'alias'; });
        this.aliasNames = this.result.data?.geneNames?.filter((geneName) => { return geneName.type === 'alias'; });
        this.location = this.result.data?.geneLocations?.find(
            (geneLocation) => {
